...
import { createBalls, holdBall, plunge, throwBall } from "@gameCore/elements/ball";
import { createLanes, createChainLanes } from "./elements/lane";
import { createChainBumpers, createCircleBumpers } from "@gameCore/elements/bumper";
...
function handleContact(contact: Contact) {
  ...
  const { gameElementType: aType, id: aId, bonus: bonusA, midPoint } = (fixtureA.getUserData() as GameElementUserData);
  const { gameElementType: bType, } = (fixtureB.getUserData() as GameElementUserData);
  ...
  if (aId === "sensor-left-drain" || aId === "sensor-right-drain") {
    holdBall(contactedBall, Vec2(12.0, 32.0), () => { throwBall(contactedBall, Vec2(0.8, -10.0)) });
  }
  ...
  if (aType === 'TUBE-SENSOR-IN') {
    ...
    const ang = Math.atan2(midPoint.y - v.y, midPoint.x - v.x) * (180 / Math.PI);
    console.log(ang);
...
function handlePreSolve(contact: Contact) {
  ...
  const contactedBall = fixtureB.getBody();
  const { gameElementType: aType, midPoint } = (fixtureA.getUserData() as GameElementUserData);

  if (aType === 'TUBE-SENSOR-IN') {

    const contactedBallPos = contactedBall.getPosition();
    const ang = Math.atan2(contactedBallPos.y - midPoint.y, contactedBallPos.x - midPoint.y);
    console.log(ang);
    const v = contactedBall.getLinearVelocity();
